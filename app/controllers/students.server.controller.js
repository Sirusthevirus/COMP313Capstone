...
exports.create = function (req, res, next) {
    ...
    console.log("body: " + req.body.studentNum);
...
exports.allUserList = function (req, res, next) {
...
exports.authenticate = function(req, res, next) {
	...
	const studentNum = req.body.auth.studentNum;
	const password  = req.body.auth.password;
	console.log('Student Number : ' + studentNum)
	console.log('Password : ' + password)
	//find the student with given studentNum using static method findOne
	Student.findOne({studentNum: studentNum}, (err, student) => {
			if (err) {
				return next(err);
			} else {
			console.log(student)
			...
			if(bcrypt.compareSync(password, student.password)) {
				...
				const token = jwt.sign({ id: student._id, studentNum: student.studentNum }, jwtKey, 
					{algorithm: 'HS256', expiresIn: jwtExpirySeconds });
				console.log('token:', token)
				// set the cookie as the token string, with a similar max age as the token
				// here, the max age is in milliseconds
				res.cookie('token', token, { maxAge: jwtExpirySeconds * 1000,httpOnly: true});
				return res.status(200).send({ screen: student.studentNum });
...
exports.welcome = (req, res) => {
	...
	// studentNum given in the token
	// use back-quotes here
	res.send(`${payload.studentNum}`)
 };
...
exports.isSignedIn = (req, res) => {
	...
	// Finally, token is ok, return the studentNum given in the token
	return res.status(200).send({ screen: payload.studentNum });
}
